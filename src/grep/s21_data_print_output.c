...
int s21_data_print_output(s21_data *setts) {
    ...
    if (setts->line[0]) {
        if (setts->line_is_match) {
            if (setts->opt_files_with_matches) {
                if (setts->files_i_is_printed == 0) {
                    printf("%s\n", setts->files[setts->files_i]);
                    setts->files_i_is_printed = 1;
                }
            } else {
                if (setts->opt_with_filename && setts->opt_no_filename == 0) {
                    printf("%s:", setts->files[setts->files_i]);
                }
                if (setts->opt_line_number) {
                    printf("%d:", setts->line_number);
                }
